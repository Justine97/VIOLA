...
output_viola_filt <- output_viola %>%
  ...
  dplyr::select(rank, VIOLA_score, X.Chrom:Type, AnnoType, Consequence, ConsDetail, GeneName, gene_id, Intron, Exon, GT, Existing_variation, Mitocarta,related_to_MD, is_unique, AF, gnomAD_AF,SIFTcat:PolyPhenVal, SpliceAI.acc.gain:MMSp_donorIntron, PHRED) %>%
  arrange(rank)
...
reactable(
  ...
  columnGroups = list(
    colGroup(name = "Annotations", columns = names(output_viola_filt)[3:18]),
    colGroup(name = "Ressources", columns = c("Gene", "Variant")),
    colGroup(name = "Minor Allele Frequency", c("AF", "gnomAD_AF")),
    colGroup(name = "Scores", columns = names(output_viola_filt)[24:36])
  ),
...
knitr::kable(output_viola_filt$Variant[1:5], escape = FALSE)

print(head(output_viola_filt$Gene))
print(head(output_viola_filt$Variant))
```
